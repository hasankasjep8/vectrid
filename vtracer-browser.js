/**
 * VTracer Browser Wrapper
 * Adapted from @visioncortex/vtracer Node.js WASM wrapper for browser use.
 * Usage:
 *   await VTracer.init();                         // load WASM (once)
 *   const svg = VTracer.vectorize_rgba(rgba, w, h, opts);
 *   const svg = VTracer.vectorize_bytes(pngData, opts);
 */
const VTracer = (() => {
  'use strict';
  ...
  // ── WASM imports (glue for wasm-bindgen) ──
  ...
  function __wbg_get_imports() {
    ...
    return { __proto__: null, "./vtracer_wasm_bg.js": import0 };
  }

  // ── Public API ──

  function vectorize_bytes(data, options) {
    if (!_initialized) throw new Error('VTracer not initialized — call await VTracer.init() first');
    ...
    try {
      ...
      if (ret[3]) { ptr2 = 0; len2 = 0; throw takeFromExternrefTable0(ret[2]); }
      deferred3_0 = ptr2;
      deferred3_1 = len2;
  ...
  async function init(wasmUrl) {
    ...
    const url = wasmUrl || 'vtracer_wasm_bg.wasm';
  ...
  return { init, vectorize_bytes, vectorize_rgba, get ready() { return _initialized; } };
})();
